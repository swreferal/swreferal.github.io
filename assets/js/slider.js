...
function sliders() {
	...
	function initSliders() {
		...
		for (var i=0;i<sliders.length;i+=1) {
			...
			// подготовьте тег <style>, который будет использоваться ползунком ручки()
	...
	function handleSlider(slider) {
		...
		// это устанавливает градиент для одного ползунка на правильный цвет остановок
		// требуется подготовленный тег <style>, созданный initSliders()
	
		var gradValue = Math.round((slider.value/slider.getAttribute('max')*1)*10000);
		var grad = 'linear-gradient(90deg,#e0505c; ' + gradValue + '%,#cccccc ' + (gradValue+1) + '%)';
